...
Mono / IL2CPP metadata string heap — searches for the structured
...
Null-terminated raw strings — some sections store plain C-style
...
# ---------------------------------------------------------------------------
# Game keyword sets - EXPANDED with anti-cheat and more game terms
# ---------------------------------------------------------------------------
...
_GAME_KEYWORDS_EXACT: frozenset[str] = frozenset({
    ...
    "lives", "score", "exp", "level", "xp", "currency", "gold", "coins",
    # movement
    "speed", "walkspeed", "runspeed", "sprintspeed", "maxspeed", "minspeed",
    "jumppower", "jumpforce", "jumpheight", "gravity", "friction",
    "acceleration", "deceleration", "velocity", "position", "rotation",
    # camera / aim
    "fov", "fieldofview", "sensitivity", "pitch", "yaw", "roll",
    "recoil", "spread", "bloom", "viewangles", "viewmatrix",
    # rendering / transforms
    "scale", "origin", "offset", "bone", "matrix", "vector", "hitbox",
    "bounds", "transform", "worldtoscreen", "screenpos",
    # weapon
    "damage", "range", "firerate", "clipsize", "magazine", "reloadtime",
    "weaponid", "bulletspeed", "projectile", "weapon", "gun",
    # engine internals
    "localplayer", "entity", "player", "character", "pawn", "playerbase",
    "int32", "float", "double", "bool", "vector3", "vector2", "quaternion",
    # Unity-specific
    "rigidbody", "collider", "animator", "networkedvar", "syncvar",
    "gameobject", "component", "behaviour", "monobehaviour",
    # Anti-Cheat & Security (NEW)
    "anticheat", "easyanticheat", "eac", "battleeye", "battleye", "be",
    "byfron", "hyperion", "vanguard", "vgc", "faceit", "ace", "tenprotect",
    "nprotect", "gameguard", "xigncode", "ahk", "cheat", "hack", "inject",
    "bypass", "detection", "security", "integrity", "validation", "verify",
    "signature", "hash", "checksum", "obfuscation", "protect", "shield",
    # Voice & Communication (NEW)
    "voice", "voicechat", "vc", "microphone", "mic", "audio", "speaker",
    "push-to-talk", "ptt", "mute", "unmute", "deafen", "undeafen",
    # Network & Multiplayer (NEW)
    "network", "multiplayer", "mp", "server", "client", "host", "peer",
    "connection", "disconnect", "latency", "ping", "packet", "rpc",
    "sync", "replication", "authority", "owner", "remote",
    # Roblox Specific (NEW)
    "roblox", "rbx", "luau", "datamodel", "workspace", "players", "player",
    "character", "humanoid", "tool", "backpack", "inventory", "leaderstats",
    "remoteevent", "remotefunction", "bindableevent", "bindablefunction",
    "instance", "property", "attribute", "tag", "collectionservice",
    # General Game Terms (NEW)
    "inventory", "item", "equipment", "loadout", "class", "role", "team",
    "faction", "guild", "party", "raid", "dungeon", "quest", "mission",
    "achievement", "trophy", "badge", "reward", "loot", "drop", "craft",
    "skill", "ability", "talent", "perk", "buff", "debuff", "status",
    "effect", "spell", "cooldown", "duration", "radius", "target",
    "npc", "enemy", "boss", "mob", "ai", "pathfinding", "navmesh",
    "collision", "trigger", "zone", "area", "region", "map", "level",
    "spawn", "respawn", "checkpoint", "save", "load", "checkpoint",
    "ui", "hud", "menu", "dialog", "tooltip", "notification", "alert",
})

# Secondary: if the field name starts with one of these prefixes it's a match
_GAME_PREFIXES: tuple[str, ...] = (
    "m_", "f_", "b_", "i_", "n_", "u_", "s_", "g_", "p_",  # common member-var conventions
    "get_", "set_", "is_", "has_", "can_", "should_",      # property accessors
    "max_", "min_", "base_", "cur_", "current_", "default_",
    "local_", "global_", "static_", "public_", "private_",
    "player_", "entity_", "game_", "world_", "server_", "client_",
    "anti_", "sec_", "auth_", "valid_", "check_",
)

# Anti-cheat process names for quick filtering (NEW)
_ANTI_CHEAT_NAMES: frozenset[str] = frozenset({
    "easyanticheat", "eac", "battleeye", "battleye", "be",
    "byfron", "hyperion", "vanguard", "vgc", "faceit", "faceitclient",
    "ace", "tenprotect", "nprotect", "gameguard", "xigncode",
    "anticheat", "cheat", "hack", "inject", "bypass",
})
...
class AnalysisConfig:
    ...
    cache_results:   bool           = True  # NEW: Enable result caching
...
# ---------------------------------------------------------------------------
# Binary pre-processing - OPTIMIZED
# ---------------------------------------------------------------------------
def extract_strings_from_bytes(data: bytes) -> str:
    ...
    OPTIMIZED: Uses pre-compiled regex and batch processing.
    """
    # Pre-allocate lists for better performance
    ascii_strings = _RE_ASCII_BYTES.findall(data)
    wide_strings = _RE_WIDE_BYTES.findall(data)
    
    # Batch decode for speed
    ascii_decoded = [s.decode("ascii", errors="ignore") for s in ascii_strings]
    wide_decoded = [s.decode("utf-16-le", errors="ignore") for s in wide_strings]
    
    return "\n".join(ascii_decoded + wide_decoded)
...
# ---------------------------------------------------------------------------
# Per-mode filtering helpers - OPTIMIZED with caching
# ---------------------------------------------------------------------------
@lru_cache(maxsize=1024)
def _is_game_field_cached(name: str) -> bool:
    """Cached version of _is_game_field for repeated lookups."""
    ...
    if any(low.startswith(p) for p in _GAME_PREFIXES):
        return True
    # Check anti-cheat names (NEW)
    if any(ac in low for ac in _ANTI_CHEAT_NAMES):
        return True
    return False
...
def _is_game_field(name: str) -> bool:
    ...
    - Contains anti-cheat related terms (NEW)
    """
    return _is_game_field_cached(name)
...
# ---------------------------------------------------------------------------
# Game mode — binary parsers - OPTIMIZED
# ---------------------------------------------------------------------------
def _parse_mono_string_heap(data: bytes) -> list[ExtractionResult]:
    ...
    OPTIMIZED: Reduced bounds checking, batch validation.
    """
    results: list[ExtractionResult] = []
    i = 0
    data_len = len(data)
    
    # Pre-compute length for faster access
    min_scan_len = 6
    
    while i < data_len - min_scan_len:
        # Quick check for null separator
        if data[i + 2] != 0x00:
            i += 1
            continue

        length = data[i + 3]
        # Faster length validation
        ...
        # Optimized ASCII check using all() with generator
...
def _scan_null_terminated_strings(data: bytes) -> list[ExtractionResult]:
    ...
    are returned.
    OPTIMIZED: Single pass with minimal branching.
    """
    results: list[ExtractionResult] = []
    i = 0
    data_len = len(data)
    ...
    while i < data_len:
        # Skip non-printable bytes quickly
        if not (0x20 <= data[i] <= 0x7E):
            i += 1
            continue

        # Collect printable run - optimized loop
        j = i
        ...
        if j < data_len and data[j] == 0x00:  # must be null-terminated
            ...
            str_len = j - i
            if 3 <= str_len <= 64:  # Reasonable field name length
                name = data[i:j].decode("ascii", errors="ignore").strip()
                if len(name) >= 3 and _is_game_field(name):
...
def _analyse_game_binary(binary: bytes) -> list[ExtractionResult]:
    ...
    OPTIMIZED: Set-based deduplication, single sort pass.
    """
...
# ---------------------------------------------------------------------------
# Main extraction engine - OPTIMIZED
# ---------------------------------------------------------------------------
def analyse(
    source: str,
    ...
    if mode == Mode.GAME:
        if binary:
            ...
            for line in source.splitlines():
                ...
                if not _is_game_field(first_word):
                    continue
...
# ---------------------------------------------------------------------------
# NEW: Batch processing helper for large files
# ---------------------------------------------------------------------------
def analyse_batched(
    source: str,
    config: AnalysisConfig,
    ...
    """
    Generator that yields results in batches for large files.
    Prevents memory issues with very large binaries.
    """
    all_results = analyse(source, config, binary)
    
    for i in range(0, len(all_results), batch_size):
        yield all_results[i:i + batch_size]